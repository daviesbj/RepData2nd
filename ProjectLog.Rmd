...
Matches <- function( pattern ){ levels( as.factor( evType[ grep( pattern, evType ) ] ) ) }
...
SetNew <- function( pattern_from, string_to ){
pattern_from = toupper( pattern_from )
string_to <- toupper( string_to )
which <- grep( pattern_from, c$EVTYPE_OLD )
c$EVTYPE_NEW[which] <- string_to
}
```

So now we do stuff like this

```{r simplify tornadoes}
Matches('TORNADO')
SumCat('TORNADO')
```
